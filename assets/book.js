...
var queued = 0;          // 翻頁中又被點的次數（最多存 2 次，賓客連點才不會沒反應）
var baseW = 0, baseH = 0;
...
function buildFaces(spread) {
  var out = [];
  out.push({ k: "cover", key: "cover" });

  A.章節.forEach(function (c, ci) {
    out.push({ k: "ch", ci: ci, key: "ch" + ci });
    (c.頁面 || []).forEach(function (p, pi) {
      var t = p.型, key = "c" + ci + "p" + pi;
      if (t === "跨頁") {
        if (spread) {
          // 跨頁必須落在「左頁(奇數) + 右頁(偶數)」，否則先補一張空白
          if (out.length % 2 === 0) out.push({ k: "blank", ci: ci, key: key + "~pad" });
          out.push({ k: "span", side: "l", p: p, ci: ci, key: key });
          out.push({ k: "span", side: "r", p: p, ci: ci, key: key + "~R" });
        } else {
          // ★ 手機直式沒有對頁，切兩半只會變成兩張很像的照片，
          //    而且還要補空白頁。單頁模式一律整張呈現。
          out.push({ k: "air", p: p, ci: ci, key: key });
        }
      } else if (t === "留白") {
        out.push({ k: "air", p: p, ci: ci, key: key });
      } else if (t === "雙直") {
        out.push({ k: "duo", p: p, ci: ci, key: key });
      } else if (t === "雙橫") {
        out.push({ k: "duh", p: p, ci: ci, key: key });
      } else if (t === "文字") {
        out.push({ k: "txt", p: p, ci: ci, key: key });
      } else if (t === "空白") {
        out.push({ k: "blank", ci: ci, key: key });
      } else {
        out.push({ k: "full", p: p, ci: ci, key: key });
      }
    });
  });

  // 結語固定排在左頁（奇數 index），這樣最後停下來時它就在眼前
  if (spread && out.length % 2 === 0) {
    out.push({ k: "blank", ci: A.章節.length - 1, key: "end~pad" });
  }
  out.push({ k: "end", ci: A.章節.length - 1, key: "end" });
...
/* 換模式（轉向）時，用 key 把讀者留在同一張照片上 */
function faceIndexByKey(list, key) {
  if (!key) return -1;
  var base = key.replace(/~(R|pad)$/, "");
  var i;
  for (i = 0; i < list.length; i++) if (list[i].key === key) return i;
  for (i = 0; i < list.length; i++) if (list[i].key === base) return i;
  return -1;
}
...
function sizeUp() {
  ...
  // 單頁模式下，書的左緣到螢幕邊那一條的寬度（翻過去的頁就露這麼多）
  var gap  = mobile ? Math.max(0, (baseW - pw) / 2) : 0;
  var edge = Math.min(34, gap);
  r.setProperty("--edge", Math.round(edge) + "px");
  r.setProperty("--mask", Math.round(gap - edge) + "px");
}
...
/* ============================================================
   ...
   ============================================================ */
function build() {
  // 換模式前先記住現在在看哪一張，換完再找回來
  var oldKey = (faces.length && faces[curFace()]) ? faces[curFace()].key : null;
  ...
  if (mobile) {
    ...
    for (i = 0; i < faces.length; i++) leaves.push(mkLeaf(faces[i], null, i));
  } else {
    for (i = 0; i < faces.length; i += 2) leaves.push(mkLeaf(faces[i], faces[i + 1] || null, i));
  }
  limit = leaves.length;

  var ni = faceIndexByKey(faces, oldKey);
  if (ni >= 0) pos = mobile ? ni : Math.floor((ni + 1) / 2);
...
function meter() {
  ...
  pgNow.textContent = f.no ? (f.no + " / " + (faces.total || 0)) : "";
...
/* ============================================================
   ...
   ============================================================ */
function go(d) {
  // 動畫中不要直接丟掉，先記著，翻完馬上補上 —— 手機連點才跟得上手
  if (animating) {
    if (Math.abs(queued) < 2) queued += d;
    return;
  }
  var to = pos + d;
